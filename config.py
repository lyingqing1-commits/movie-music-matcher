...
# 各剪辑平台草稿目录（自动检测）
def _detect_jianying_draft_dir() -> str:
    """检测剪映专业版（中国版）草稿目录"""
    path = os.path.expandvars(r"%LOCALAPPDATA%\JianyingPro\User Data\Projects\com.lveditor.draft")
    parent = os.path.dirname(path)
    return path if os.path.exists(parent) else None

def _detect_capcut_draft_dir() -> str:
    """检测 CapCut（国际版）草稿目录"""
    path = os.path.expandvars(r"%LOCALAPPDATA%\CapCut\User Data\Projects\com.lveditor.draft")
    parent = os.path.dirname(path)
    return path if os.path.exists(parent) else None

JIANYING_DRAFT_DIR = _detect_jianying_draft_dir()
CAPCUT_DRAFT_DIR = _detect_capcut_draft_dir()

# DaVinci Resolve 输出目录（默认在 output 下创建 resolve 子目录）
RESOLVE_OUTPUT_DIR = os.path.join(BASE_DIR, "output", "resolve")

# 通用蓝图输出目录
UNIVERSAL_OUTPUT_DIR = os.path.join(BASE_DIR, "output", "universal")
...
# 🔐 管理员配置
# ============================================
ADMIN_KEY = _env("ADMIN_KEY", "")  # 管理员密钥（在 .env 中设置 ADMIN_KEY=你的密钥），用于意见箱管理等操作
...
FORCE_MANUAL_DRAFT = True           # True=手动JSON模式（3轨分离更可靠），False=pyJianYingDraft
