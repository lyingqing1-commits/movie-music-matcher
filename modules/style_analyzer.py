...
def analyze_style(frame_paths: list[str]) -> dict:
    """
    分析电影画面风格
    ...
    # ---- 方法 1：尝试 Vision API ----
    try:
        print(f"AI 正在分析 {len(frame_paths)} 帧画面（Vision 模式）...")
    ...
    # ---- 方法 2：回退到元数据模式 ----
    return _analyze_by_metadata(frame_paths, client)


def _analyze_by_metadata(frame_paths: list[str], client: Anthropic) -> dict:
    """
    ...
    # 构建纯文本的分析请求
    user_message = f"""请根据以下电影帧画面的数值特征分析其风格：
