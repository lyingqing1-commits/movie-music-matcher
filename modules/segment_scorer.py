...
SYSTEM_PROMPT_SEGMENT_SCORER_BASE = """你是一位资深视频剪辑师，擅长为影视混剪/高光集锦挑选最佳片段。
...
# 视频优先模式的评分指引
VIDEO_FIRST_INSTRUCTION = """【剪辑模式：视频优先】
当前剪辑模式为「视频优先」——最高光的片段将出现在音乐最重要的段落。

评分侧重：
- 更严格地区分 highlight_score：真正的高光时刻应得90+，普通画面应在40-60
- suggested_use 的准确性至关重要：明确标注每个片段最适合的音乐段落位置
- 动作强度、情绪冲击力、视觉张力是核心评判标准
- 避免给过渡性画面打高分"""

# 音乐优先模式的评分指引
MUSIC_FIRST_INSTRUCTION = """【剪辑模式：音乐优先】
当前剪辑模式为「音乐优先」——视频跟随音乐的情感弧线，每个音乐段落匹配最合适的情绪。

评分侧重：
- emotional_tone 的准确性至关重要：必须精确标注每个片段的情感基调
- 情感多样性能让MV更丰富——请尽量分散使用不同的 emotional_tone
- highlight_score 仍然重要，但情感匹配度优先
- 注意片段的"情感可连接性"：悲伤后接希望、紧张后接释放等"""
...
def score_segments(
    ...
    if not config.ANTHROPIC_API_KEY:
        ...
        return _default_scores(segments, editing_mode)
...
请结合你对这部电影的了解来评估每个片段的重要性和情感基调。"""
        ...
        if editing_mode == "music_first":
            mode_instruction = MUSIC_FIRST_INSTRUCTION
        else:
            mode_instruction = VIDEO_FIRST_INSTRUCTION

        system_prompt = SYSTEM_PROMPT_SEGMENT_SCORER_BASE.format(
...
def _default_scores(segments: list[dict], editing_mode: str = "video_first") -> list[dict]:
    """默认评分（AI 不可用时的回退）——根据剪辑模式调整分布"""
    import random
    ...
    if editing_mode == "music_first":
        # 音乐优先：情感多样化
        uses_weights = {"drop": 1, "chorus": 2, "verse": 3, "intro": 1, "bridge": 2, "outro": 1}
        # 让情感分布更均衡
        tone_weights = [2, 2, 1.5, 2, 1.5, 1.5, 1.5, 1, 1, 1.5]
        score_range = (35, 80)
    else:
        # 视频优先：高分片段集中在 drop/chorus
        uses_weights = {"drop": 3, "chorus": 2, "verse": 1, "intro": 1, "bridge": 1, "outro": 1}
        tone_weights = [2.5, 1.5, 1.5, 1, 2, 1, 1, 1, 1, 2]  # action, climax 偏重
        score_range = (40, 90)

    uses_pool = []
    for u, w in uses_weights.items():
        uses_pool.extend([u] * int(w * 3))
    random.shuffle(uses_pool)

    for i, seg in enumerate(segments):
        seg["highlight_score"] = random.randint(score_range[0], score_range[1])
        seg["emotional_tone"] = random.choices(tones, weights=tone_weights, k=1)[0]
        seg["suggested_use"] = uses_pool[i % len(uses_pool)] if uses_pool else "verse"
        seg["visual_quality"] = "medium"
        seg["description"] = "（默认评分）"
        seg["_default_scored"] = True

    print(f"   [WARN] 使用默认评分 ({len(segments)} 个片段, 模式: {editing_mode})")
    return segments
